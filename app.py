...
- Change 1
    Added Mark_Circle insted of mark_point
- Change 2
    Changing Color of circl based on X values
- Change 3
    Changing Size based on Y values
- Change 4 
    Added Tooltip to show x and y values
- Change 5
    Added .interactive() so you can click on chart and drag chart left-right or up-down
""")

scatter = alt.Chart(df).mark_circle().encode(x='x',
    y='y',size='y', color='x',tooltip=['x', 'y']).interactive()
...
- Change 1
    Converted Cylinders column type to get different colors for each value
- Change 2
    Added Name and Origin to tooltip to get specific information along with interactive option
"""
)
...
strip = alt.Chart(cars).mark_tick().encode(
    ...
    y='Cylinders:O',
    color='Cylinders:N',
    tooltip=['Name', 'Origin']).interactive()
